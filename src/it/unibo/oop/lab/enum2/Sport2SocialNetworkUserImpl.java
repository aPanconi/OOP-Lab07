...
public class Sport2SocialNetworkUserImpl<U extends User> extends SocialNetworkUserImpl<U> {
     ...
     */
    final private Set<Sport> sports;
	...
	for (final Sport elem : this.sports) {
	    if (elem == s) {
		return true;
	    }
	}
     ...
     */
	  ...
	  final Set<Sport> individuals = new HashSet<>();
     ...
     */
	  ...
	  final Set<Sport> byPlace = new HashSet<>();
	  
	  for (final Sport elem: this.sports) {
	      if (elem.isIndoorSport() && p == Place.INDOOR) {
		  byPlace.add(elem);
	      } else if (!elem.isIndoorSport() && p == Place.OUTDOOR) {
		  byPlace.add(elem);
	      }
	  }
	  return byPlace; 
